...
import commons
from ljspeech import parse_ljspeech
...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        for audiopath, text in self.audiopaths_and_text:
            if (
                len(text) >= self.hparams.min_text_len
                and len(text) <= self.hparams.max_text_len
            ):
    ...
    def get_audio(self, filename: Path):
        ...
        if self.hparams.use_mel_posterior_encoder:
            spec_filename = filename.with_suffix(".mel.pt")
