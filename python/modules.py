...
class CBHG(nn.Module):
    def __init__(self, dim_input, dim_output, dim_rnn=128, dim_conv_hidden=128, dim_proj_hidden=128, dim_highway=128, K=16):
        super().__init__()
        ...
        self.conv_proj = nn.Sequential(
            nn.Conv1d(len(self.conv_bank)*dim_conv_hidden, dim_proj_hidden, kernel_size=3, bias=False),
        ...
        self.highway = nn.Sequential(
            nn.Linear(dim_input, dim_highway, bias=False) if dim_input != dim_highway else nn.Identity(),
            HighwayLayer(dim_highway, dim_highway, nn.functional.relu),
            HighwayLayer(dim_highway, dim_highway, nn.functional.relu),
            HighwayLayer(dim_highway, dim_highway, nn.functional.relu),
            HighwayLayer(dim_highway, dim_highway, nn.functional.relu)
        )
        self.rnn = nn.GRU(input_size=dim_highway, hidden_size=dim_rnn, bidirectional=True, batch_first=True)
...
class ContentAttention(nn.Module):
    ...
    def forward(self, score, context):
        ...
        c = torch.bmm(w.unsqueeze(1), context) # (B x 1 x L_ctx) x (B x L_ctx x D_ctx) -> (B x 1 x D_ctx)
