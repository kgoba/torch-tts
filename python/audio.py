...
class AudioFrontend:
    def __init__(self, config):
        ...
        self.mels_to_stft = torchaudio.transforms.InverseMelScale(n_mels=self.config.num_mels, sample_rate=self.config.sample_rate, n_stft=n_stft, 
                                                           f_min=self.config.fmin, f_max=self.config.fmax, norm="slaney")

    def encode(self, wave, sr):
        if sr != self.config.sample_rate:
            wave = torchaudio.functional.resample(wave, orig_freq=sr, new_freq=self.config.sample_rate)
        D = torchaudio.functional.spectrogram(wave, n_fft=self.n_fft, hop_length=self.hop_length, power=1)
    ...
    def decode(self, D_db):
        ...
        return torchaudio.functional.griffinlim(D, hop_length=self.hop_length, power=1, momentum=0.9), self.config.sample_rate
