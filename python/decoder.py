...
class DecoderTeacher(nn.Module):
    ...
    def forward(self, x, encoder_outputs):
        ...
        h_att_t = torch.zeros(
            (B, self.decoder_cell.dim_att), dtype=x.dtype, layout=x.layout, device=x.device
        )
        h_dec_t = [
            torch.zeros(
                (B, self.decoder_cell.dim_rnn), dtype=x.dtype, layout=x.layout, device=x.device
            )
            for _ in range(self.decoder_cell.decoder_depth)
        ]

        y_t = torch.zeros(
            (B, self.r, self.decoder_cell.dim_mel), dtype=x.dtype, layout=x.layout, device=x.device
        )  # GO frame
        w_t = torch.stack(
            torch.ones((B, 1), dtype=x.dtype, layout=x.layout, device=x.device),
            torch.zeros(
                (B, encoder_outputs.shape(1) - 1), dtype=x.dtype, layout=x.layout, device=x.device
            ),
            dim=1,
        )  # Initial attention weights
        y, w = [], []
        for x_t in x.tensor_split(self.r, dim=1):
            y_t, w_t, h_att_t, h_dec_t = self.decoder_cell(
